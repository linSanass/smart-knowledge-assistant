...
- **多会话**：新建 / 切换 / 删除，会话列表按更新时间排序。
  点「新建会话」只是进入草稿状态，**没发过消息的会话不会出现在列表里** ——
  真正的创建发生在发出第一条消息时
...
- **四种模式**，同一个接口按 `mode` 切换：
  ...
  前端只自动使用 `chat` 与 `rag`（按知识库是否就绪切换）；
  `tools` 与 `agent` 仍然可用，通过接口调用，见下方 API 简介
...
- 单个文件损坏不影响其它条目，失败原因写回该条目的 `status` 与 `error`
...
1. 用输入框左侧的回形针上传 PDF / TXT / Markdown（可多选）
2. 上传后会自动在后台构建知识库，侧栏的状态会实时更新
3. 构建完成后直接提问即可。**模式是自动选的**：
   知识库已就绪就走知识库问答（回答下方列出参考来源），
   否则走普通对话 —— 界面上没有模式切换按钮
4. 也可以点侧栏的「新建知识」手写一条知识，保存后同样进入索引；
   点击列表里的任一条目可看详情、编辑或删除
...
1. **backend 只能单进程**。FAISS 索引与构建状态都是进程内内存态，
   ...
   如需多副本，得先把索引外置。
2. **uploads 用 bind mount**。上传的文件落在宿主机 `backend/uploads/`，
   容器重建不丢；删容器不会删文件。
...
共 27 个端点。完整文档见 <http://127.0.0.1:8000/docs>。
